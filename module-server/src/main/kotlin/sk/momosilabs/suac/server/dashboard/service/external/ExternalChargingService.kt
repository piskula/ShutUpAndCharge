...
open class ExternalChargingService(
    ...
    override fun getChargerStatus(): ExternalChargerDataWrapper {
        val response = connectionToStation(applicationProperties.station)
            .retrieve()
            .toEntity(String::class.java)

        val isOk = response.statusCode == HttpStatus.OK
        return ExternalChargerDataWrapper(
            error = !isOk,
            success = isOk,
            ifSuccess = if (isOk) deserializer.decodeFromString<ExternalChargerSuccessResponse>(response.body!!).toModel() else null,
            ifError = null,
        )
    }
