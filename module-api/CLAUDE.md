...
File download endpoints must use `format: "binary"` (not `"byte"`) in the OpenAPI spec — `"byte"` means a Base64 string in a JSON body, `"binary"` means raw binary stream. The generated Angular client produces `Observable<Blob>` for `"binary"` responses.
